...
Поддерживает четыре типа документов: руководство пользователя, руководство администратора, руководство оператора и техническое описание. Документы создаются в Markdown и конвертируются в DOCX через pandoc с оформлением по ГОСТ 2.105. Скриншоты интерфейса делаются автоматически через Playwright.
...
Скилл задаст несколько вопросов: какие документы генерировать, режим ГОСТ (строгий или облегчённый), путь к ТЗ, URL приложения для скриншотов. После этого запустит субагентов для анализа кода и спецификаций, сделает скриншоты через Playwright и сгенерирует документы.
...
2. Исследование — два субагента параллельно анализируют код и ТЗ, затем третий делает скриншоты
